...
def authorized_only(func):
    ...
    def wrapper(client, message, *args, **kwargs):
        ...
        if user_id not in AUTH_USERS:
            client.send_message(
                message.chat.id,
                f"🚫 **Unauthorized Access**: You are not allowed to use this bot.",
...
def add_user(client, message):
    try:
        ...
        if new_user_id not in AUTH_USERS:
            ...
            bot.send_message(
                message.chat.id,
                f"✅ **User {new_user_id} added to authorized users.**",
                reply_to_message_id=message.id
            )
        else:
            bot.send_message(
                message.chat.id,
                f"⚠️ **User {new_user_id} is already authorized.**",
                reply_to_message_id=message.id
            )
...
def remove_user(client, message):
    try:
        ...
        if remove_user_id in AUTH_USERS:
            AUTH_USERS.remove(remove_user_id)
            bot.send_message(
                message.chat.id,
                f"✅ **User {remove_user_id} removed from authorized users.**",
                reply_to_message_id=message.id
            )
        else:
            bot.send_message(
                message.chat.id,
                f"⚠️ **User {remove_user_id} is not in authorized users.**",
                reply_to_message_id=message.id
            )
    except (IndexError, ValueError):
        bot.send_message(
